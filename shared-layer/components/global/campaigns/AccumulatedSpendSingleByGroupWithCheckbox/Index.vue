...
const { isComplete, percentage, progressByGroup } = useProgramProgress({
  programInfo,
    ...
    'relative flex h-full items-center justify-center border-l border-border-neutral px-4 font-bold',
    {
      variants: {
        status: {
          ...
          unclaimed: 'bg-primary font-bold text-white hover:bg-primary/90 dark:bg-primary-dark',
...
<template>
  <div :class="cn('flex flex-col gap-6', classes?.outer)">
    ...
    <template v-else>
      <div
        v-if="mappedProgress"
        :class="cn(wrapperVariants({ status: mappedProgress.status }), classes?.wrapper)"
      >
        <div class="flex flex-col gap-5 px-5 py-6">
          <div class="flex gap-5">
            ...
            <div class="flex grow items-center gap-3">
              ...
              <div :class="checkboxVariants({ status: mappedProgress.status })">
                <Icon
          ...
          <div class="flex items-center justify-between gap-1">
            ...
            <Progress v-model="percentage" class="grow [&>div]:bg-success" />
          ...
          <div class="flex items-center justify-between text-xs">
            ...
            <span>
              <template v-if="programInfo?.description">{{ programInfo.description }}</template>
              <template> as of <NuxtTime :datetime="Date.now()" /> </template>
            </span>
          </div>
        </div>
            ...
            <template v-if="mappedProgress.hasRedemption">
              ...
              >
                <button
                  ...
                  class="relative z-10 h-full w-full disabled:cursor-not-allowed"
                >
                  <Icon
                    v-if="isPending"
                  ...
                  <span v-else class="relative z-10 size-full">
                    {{ mappedActionLabel[mappedProgress.status] }}
                  </span>
                </button>
              </RenderlessRewardRedemption>
