...
interface Classes extends NestedClasses<TransactionTableProps> {
...
<template>
  <div :class="cn('min-h-60', classes?.outer)">
    ...
    <template v-else-if="isError && alerts.length">
      <alert-hero-centered :alert="alerts[0]" />
    </template>
    ...
    <template v-else>
      ...
      >
        <div class="basis-2/3 truncate text-left">
          <p class="overflow-hidden font-bold text-ellipsis text-text-primary">
        ...
        <div class="shrink-0 basis-1/3 text-right">
          <p class="font-semibold text-text-primary">P{{ transaction.amount.toLocaleString() }}</p>

          <p class="mt-2 text-xs whitespace-nowrap text-text-secondary">
